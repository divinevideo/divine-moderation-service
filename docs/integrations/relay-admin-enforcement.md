...
The only caller that can be refused is `POST /admin/api/uploader/:pubkey/enforcement`, reached from the dashboard's Ban/Unban User button. (`deleteRelayEventIds` calls `callRelayAdminAction` too, but only with `delete_event`, which is unguarded.) A refusal returns before `setUploaderEnforcement` runs, so no local row records an enforcement the relay never applied — including any `approvalRequired`/`notes` submitted in the same request. The guarantee is only as good as the failure being detected: `callRelayAdminAction` treats a 2xx whose body will not parse as JSON as success, so a relay answering `200` with HTML would still be recorded locally.

On a `409` the dashboard shows the refusal with an **Open case** action that opens `caseUrl` — but only when `caseUrl` is `https:`, since it is handed to `window.open`. Anything that leaves the dashboard without an `https:` `caseUrl` drops both the action and the case id, falling back to the same `User action failed: <message>` toast: a non-https `RELAY_ADMIN_UI_URL`, and equally a `caseId` this service could not trust (non-string or blank), since `caseUrl` is only built when the id survived. On a `503` it shows `User action failed: <relay-admin's message>` — the same plain red toast as a `502`, so the two are not distinguishable in the UI beyond whatever the upstream message happens to say. Nothing here retries automatically.
