...
```
Logic: cursor → poll since → group by target video → cap at batch limit (log rollover) → per video: fetch video event (skip if missing), fetch all its label events, decide via Task 5, skip labels with existing decisions, publish → record decision (+vote count, sha256 from video event tags) → record strike if warranted → warning check (count, once per level) → DM. Advance cursor only if every touched video processed without transient error; on partial failure keep cursor (idempotency via PKs makes re-sweep safe).
